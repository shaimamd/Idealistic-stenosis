...
fptr_MD_2 = open("WSSdatacyl.txt", "w")
#The vector rotation is given by v'=R.v
#Second order tensor rotation is given as: A'=R A transpose(R)
for il in range(len(pwal)):
     ti=il%15
     R=np.matrix([[np.cos(angle[ti]*np.pi/180),np.sin(angle[ti]*np.pi/180),0],[-np.sin(angle[ti]*np.pi/180),np.cos(angle[ti]*np.pi/180),0],[0,0,1]])
     ...
     if rl==0:
        n=np.array([-data_pos[0],-data_pos[1],0])
        nc=np.dot(R,n)
     else: 
